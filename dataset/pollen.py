...
class PollenDataSet(data.Dataset):
    ...
    def __init__(self, root='', list_path='', max_iters=None, transform=None):
        ...
        self.img_ids = [i_id.strip() for i_id in open(list_path, encoding='utf8')]
        if not max_iters == None:
            self.img_ids = self.img_ids * int(np.ceil(float(max_iters) / len(self.img_ids)))
        self.files = []
        self.transform = transform

        self.vocab = pickle.load(open('dataset/pollen/vocab.pkl', 'rb'))
        with torch.no_grad():
            embedding_pretrained1 = torch.tensor(
                np.load('dataset/pollen/embedding_M30_300d.npz')["embeddings"].astype('float32'))
            self.embedding1 = nn.Embedding.from_pretrained(embedding_pretrained1, freeze=True)
            embedding_pretrained2 = torch.tensor(
                np.load('dataset/pollen/embedding_Wiki_300d.npz')["embeddings"].astype('float32'))
            self.embedding2 = nn.Embedding.from_pretrained(embedding_pretrained2, freeze=True)
            self.embedding3 = nn.Embedding(len(self.vocab), 300, padding_idx=len(self.vocab) - 1)

        for name in self.img_ids:
            img_file, label_file = name.split('\t')
            self.files.append({
                "img": img_file,
                "label": label_file.rstrip('\n'),
                "name": name,
            })
    ...
    def __getitem__(self, index):
        ...
        # print(datafiles)
        ids, pad_size = get_ids(datafiles["img"], char2id=self.vocab)
        image = torch.stack([self.embedding1(torch.LongTensor(ids)),
                             self.embedding2(torch.LongTensor(ids)),
                             self.embedding1(torch.LongTensor(ids))])
        label = np.ndarray(self.embedding1(torch.LongTensor(ids)).shape)
        label.fill(int(datafiles["label"]))
...
class PollenValDataSet(data.Dataset):
    ...
    def __init__(self, root='', list_path='', transform=None):
        ...
        self.vocab = pickle.load(open('dataset/pollen/vocab.pkl', 'rb'))
        with torch.no_grad():
            embedding_pretrained1 = torch.tensor(np.load('dataset/pollen/embedding_M30_300d.npz')["embeddings"].astype('float32'))
            self.embedding1 = nn.Embedding.from_pretrained(embedding_pretrained1, freeze=True)
            embedding_pretrained2 = torch.tensor(np.load('dataset/pollen/embedding_Wiki_300d.npz')["embeddings"].astype('float32'))
            self.embedding2 = nn.Embedding.from_pretrained(embedding_pretrained2, freeze=True)
            self.embedding3 = nn.Embedding(len(self.vocab), 300, padding_idx=len(self.vocab) - 1)
...
class PollenTestDataSet(data.Dataset):
...
def get_ids(sent, char2id, pad_size=32):
    ret = []
    token = sent.split(' ')
    if pad_size:
    ...
    # word to id
    for word in token:
        ret.append(char2id.get(word, char2id.get('<UNK>')))
    return ret[:pad_size], pad_size


def gen_part_list_txt(t_path="img_train", m_path="lab_train", z_path="img_testA"):
    lst = []
    print("./pollen/"+t_path)
    for name in os.listdir("./pollen/"+t_path):
    ...
    test_lst = []
    for name in os.listdir("./pollen/"+z_path):
    ...
    print(len(test_lst))
    m = 0.95
    with open('./pollen/pollen_train_list.txt', mode='w') as f:
        for x in lst[: int(m*len_lst)]:
            f.write(x + "\n")
    with open('./pollen/pollen_val_list.txt', mode='w') as f:
        for x in lst[int(m * len_lst):]:
            f.write(x + "\n")
    with open('./pollen/pollen_test_list.txt', mode='w') as f:
        for x in test_lst:
            f.write(x + "\n")
    with open('./pollen/pollen_trainval_list.txt', mode='w') as f:
        for x in lst:
            f.write(x + "\n")
...
def gen_test_list_txt(z_path="img_train"):
    ...
    print(len(test_lst))
    m = 0.99
    with open('./pollen/pollen_testB_list.txt', mode='w') as f:
        for x in test_lst:
            f.write(x + "\n")
